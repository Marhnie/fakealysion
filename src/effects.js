  ...
  switch (instr.op) {
    ...
    case 'setDP': {
      ...
      if (targetUid) {
        ...
        S.modifyDP(state, targetPlayer, targetUid, instr.value - base - (stack.tempDP || 0), instr.duration || 'turn');
      }
      break;
    }
    case 'restrictAttack': {
...
export function compileToScript(text) {
  ...
  // Blocker / Jamming / Piercing keyword grants.
  for (const [kw, re] of [['블로커', /[≪《]\s*블로커\s*[≫》]/], ['재밍', /[≪《]\s*재밍\s*[≫》]/], ['관통', /[≪《]\s*관통\s*[≫》]/]]) {
