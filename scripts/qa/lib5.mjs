...
export async function drain(st, choose) {
  ...
  while (guard++ < 60) {
    ...
    if (t.stackUid && t.topId && !t.evt?.leaving && !t.tags.some(x => x.includes('소멸 시'))) { const n = fs_(t.stackUid); let why = null; if (!n || n.cardId !== t.topId) why = 'left'; else if (t.inherited && !n.sources.includes(t.cardId)) why = 'lost inherited'; else if (t.tags.includes('자신의 턴') && t.player !== st.activePlayer) why = 'not own turn'; else if (t.tags.includes('상대의 턴') && t.player === st.activePlayer) why = 'not opp turn'; if (why) { ran.push(`SKIP ${t.cardId} (${why})`); S.resolvePending(st, t.uid); continue; } }
